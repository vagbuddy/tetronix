  ...
  switch (action.type) {
    ...
    case 'PLACE_PIECE': {
      ...
      if (!piece && action.pieceId) {
        piece = state.availablePieces.find(p => p.instanceId === action.pieceId) || null;
      }
      ...
      const newGrid = placePieceOnGrid(pieceToPlace, action.position, state.grid);
      
      // Update the piece as placed
      const updatedPieces = state.availablePieces.map(p => 
        p.instanceId === pieceToPlace.instanceId ? { ...p, isPlaced: true, position: action.position } : p
      );
      ...
      scoreIncrease = calculateScore(linesIncrease, fullSudokuBlocks.length, state.level);
      
      // Check if all pieces are placed - if so, generate new pieces
      const allPiecesPlaced = updatedPieces.every(p => p.isPlaced);
      const finalPieces = allPiecesPlaced ? generateRandomPieces() : updatedPieces;
    ...
    case 'ROTATE_PIECE': {
      ...
      return {
        ...
        selectedPiece: state.selectedPiece?.instanceId === action.pieceId ? 
          rotatePiece(state.selectedPiece) : state.selectedPiece
      };
    }
    
    case 'START_DRAG': {
      const updatedPieces = state.availablePieces.map(piece => 
        piece.instanceId === action.piece.instanceId ? { ...piece, isDragging: true } : piece
      );
...
export const useGameState = () => {
  ...
  const placePiece = useCallback((position: { x: number; y: number }, pieceId?: string) => {
