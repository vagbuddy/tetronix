import { Cell, Position, TetrisPiece, GameState, SudokuBlock } from '../types/GameTypes';
...
export const TETRIS_PIECES: Omit<TetrisPiece, 'position' | 'rotation' | 'isPlaced' | 'isDragging' | 'instanceId'>[] = [
  {
    id: 'I',
    shape: [
      [1, 1, 1],
...
export const createRandomPiece = (): TetrisPiece => {
  ...
  return {
    ...pieceTemplate,
    instanceId: `${pieceTemplate.id}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
...
export const generateRandomPieces = (): TetrisPiece[] => {
  return Array(PIECE_SELECTION_COUNT).fill(null).map(() => createRandomPiece());
};
...
export const getPieceCells = (piece: TetrisPiece, position?: Position): Position[] => {
...
export const isValidPlacement = (piece: TetrisPiece, position: Position, grid: Cell[][]): boolean => {
...
export const placePieceOnGrid = (piece: TetrisPiece, position: Position, grid: Cell[][]): Cell[][] => {
...
export const clearSudokuBlocks = (grid: Cell[][], blocks: SudokuBlock[]): Cell[][] => {
  let newGrid = grid.map(row => row.map(cell => ({ ...cell })));
...
export const calculateScore = (linesCleared: number, sudokuBlocksCleared: number, level: number): number => {
  const lineScore = linesCleared * 100 * level;
  const blockScore = sudokuBlocksCleared * 500 * level;
  return lineScore + blockScore;
};

export const getGridPositionFromMouse = (mouseX: number, mouseY: number, boardRect: DOMRect): Position | null => {
