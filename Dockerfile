# ---------- Frontend build ----------
FROM node:20-slim AS frontend-build
WORKDIR /app

ARG BUILD_ENV=production
ARG VITE_API_URL=""
ENV VITE_API_URL=${VITE_API_URL}
ARG VITE_ENABLE_ADMIN="false"
ENV VITE_ENABLE_ADMIN=${VITE_ENABLE_ADMIN}
ARG VITE_LEADERBOARD_SUBMIT_COOLDOWN_MS=""
ENV VITE_LEADERBOARD_SUBMIT_COOLDOWN_MS=${VITE_LEADERBOARD_SUBMIT_COOLDOWN_MS}
ARG VITE_START_GAME_TIMEOUT_MS=""
ENV VITE_START_GAME_TIMEOUT_MS=${VITE_START_GAME_TIMEOUT_MS}

# Firebase Config Args
ARG VITE_FIREBASE_API_KEY=""
ENV VITE_FIREBASE_API_KEY=${VITE_FIREBASE_API_KEY}
ARG VITE_FIREBASE_AUTH_DOMAIN=""
ENV VITE_FIREBASE_AUTH_DOMAIN=${VITE_FIREBASE_AUTH_DOMAIN}
ARG VITE_FIREBASE_PROJECT_ID=""
ENV VITE_FIREBASE_PROJECT_ID=${VITE_FIREBASE_PROJECT_ID}
ARG VITE_FIREBASE_APP_ID=""
ENV VITE_FIREBASE_APP_ID=${VITE_FIREBASE_APP_ID}
ARG VITE_FIREBASE_RECAPTCHA_SITE_KEY=""
ENV VITE_FIREBASE_RECAPTCHA_SITE_KEY=${VITE_FIREBASE_RECAPTCHA_SITE_KEY}
ARG VITE_FIREBASE_APPCHECK_DEBUG_TOKEN=""
ENV VITE_FIREBASE_APPCHECK_DEBUG_TOKEN=${VITE_FIREBASE_APPCHECK_DEBUG_TOKEN}

COPY package*.json ./
RUN npm install --no-audit --no-fund

COPY . .
RUN npm run build


# ---------- API build ----------
FROM node:20-slim AS api-build
WORKDIR /app/api

COPY api/package*.json ./
RUN npm install --no-audit --no-fund
COPY api .
# Copy shared code into the build context so the API tsc can import from ../shared
COPY shared ../shared
RUN npm run build


# ---------- Runtime ----------
FROM node:20-slim AS runtime
ARG BUILD_ENV=production
ARG APP_CHECK_ENFORCE=true
# Allow overriding the listening port at build time
ARG PORT=80
ENV NODE_ENV=${BUILD_ENV}
ENV APP_CHECK_ENFORCE=${APP_CHECK_ENFORCE}
ENV PORT=${PORT}
WORKDIR /app/api

COPY --from=frontend-build /app/build /app/api/build

COPY api/package*.json ./
RUN npm install --omit=dev --no-audit --no-fund

COPY --from=api-build /app/api/dist ./dist

EXPOSE ${PORT}
CMD ["node", "dist/api/index.js"]
